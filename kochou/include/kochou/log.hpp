#ifndef KOCHOU_LOG_HPP
#define KOCHOU_LOG_HPP

#include <concepts>
#include <format>
#include <iostream>
#include <source_location>
#include <string_view>

#include <ktl/errc.hpp>
#include <ktl/flat_set.hpp>

namespace ktl
{
using log_tim_type = std::chrono::time_point< std::chrono::system_clock >;
using log_ctx_type = std::source_location;
using log_msg_type = std::string;
using log_lvl_type = ktl::err_level;
using log_err_type = ktl::errc;

struct log_data final
{
    log_tim_type tim;
    log_ctx_type ctx;
    log_msg_type msg;
    log_lvl_type lvl;
    log_err_type err;
};

enum class log_level
{
    debug   = 0,
    info    = 1,
    warning = 2,
    error   = 3,
    fatal   = 4
};

[[maybe_unused]] constexpr std::string
format_log(const log_data &);

[[maybe_unused]] constexpr std::string
format_log(const log_tim_type &, const log_ctx_type &, const log_msg_type &, const log_lvl_type &,
           const log_err_type &);

#ifdef KOCHOU_BUILD_DEBUG
[[maybe_unused]] inline void
log(log_level _level, std::string_view _msg, log_ctx_type _ctx = log_ctx_type::current())
{
    switch (_level)
    {
    case log_level::debug:
        break;
    case log_level::info:
        break;
    case log_level::warning:
        break;
    case log_level::error:
        break;
    case log_level::fatal:
        break;
    default:
        break;
    }
}
#elifdef KOCHOU_BUILD_RELEASE
[[maybe_unused]] inline void
log(log_level, std::string_view, log_ctx_type)
{
}
#else
static_assert(false && "kochou build mode unspecified!");
#endif
} // namespace ktl

#endif
