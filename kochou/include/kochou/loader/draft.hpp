...
std::ostream &
operator<<(std::ostream & out, const application_info & _application_info) noexcept
{
    std::osyncstream synced_out(out);
    return synced_out << "kochou::core::application_info\n"
...
template <>
struct std::formatter< application_info > : std::formatter< std::string >
{
    auto
    format(application_info _application_info, format_context & _format_context) const noexcept
    {
        return std::formatter< std::string >::format(
            std::format(
                ...
                core::enum_cast< core::structure_type >(_application_info.type),
                _application_info.next, _application_info.application_name, _application_info.application_version,
                _application_info.engine_name, _application_info.engine_version, _application_info.api_version),
            _format_context);
    }
};
} // namespace kochou::loader
...
{
enum class structure_type : ktl::mask_underlying_type // uint32_t
{
    application_info = 0
    // ....          = 1
};
...
{
    constexpr std::string
    operator()(structure_type _structure_type) noexcept
    {
