...
vk::utils::window::window(std::size_t width, std::size_t height, std::string_view title) : __window(nullptr)
{
    if (!glfwInit())
    {
...
vk::utils::window::create_surface(vk::raii::Instance & instance)
{
    ...
    if (result != VK_SUCCESS || !surface)
    {
        ...
        throw std::runtime_error("window | surface is null! result: " + std::to_string(result) + description +
                                 std::to_string(code));
    }
    return surface;
}
