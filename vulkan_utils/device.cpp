...
vk::utils::device::device(vk::raii::Instance & instance, vk::raii::SurfaceKHR & surface, bool is_debug)
    : __physical_device(VK_NULL_HANDLE), __device(VK_NULL_HANDLE), __graphics_queue(VK_NULL_HANDLE),
      __present_queue(VK_NULL_HANDLE)
{
    ...
    device_extensions = {"VK_KHR_swapchain", "VK_KHR_portability_subset"};
#else
    ...
    if (is_debug)
    {
        device_layers = {"VK_LAYER_KHRONOS_validation"};
