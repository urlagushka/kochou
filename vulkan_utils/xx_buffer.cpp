...
vk::utils::create_buffer(const device & dev, std::size_t size, vk::BufferUsageFlags uf, vk::MemoryPropertyFlags mf)
{
    ...
    vk::MemoryAllocateInfo memory_info = {memory_req.size, dev.find_memory_type(memory_req.memoryTypeBits, mf)};
...
vk::utils::create_index_buffer(const device & dev, const std::vector< uint32_t > & indices)
{
    ...
    auto [buffer, memory] =
        create_buffer(dev, size, vk::BufferUsageFlagBits::eIndexBuffer,
...
{
    return create_buffer(dev, size, vk::BufferUsageFlagBits::eUniformBuffer,
                         vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent);
}
...
vk::utils::create_framebuffers(const device & dev, render_pass & rd, swapchain & sw)
{
    ...
    {
        std::array< vk::ImageView, 2 > atchs = {*image_view, *depth_view};
...
{
    vk::CommandBufferAllocateInfo alloc_info{*pool, vk::CommandBufferLevel::ePrimary, size};
...
{
    vk::CommandPoolCreateInfo cmd_pool_info{vk::CommandPoolCreateFlagBits::eResetCommandBuffer,
